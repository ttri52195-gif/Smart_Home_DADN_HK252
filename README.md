...
### 2026-06-05 (latest)
- **ThresholdSettingsScreen — role-based access**: fetches the user's profile via `GET /get-user-by-username` at load time. Home Owners see fully editable fields and the Save Changes button as before. Members see a yellow lock banner ("Only the Home Owner can change threshold settings"), read-only inputs (50% opacity, `editable={false}`), and no Save button. Both roles load threshold values from the same `GET /api/setting-profiles/current/thresholds` endpoint.
- **ForgotPasswordScreen — username + email required**: step 1 now collects both username and email; both are sent to `POST /api/auth/forgot-password/verify-email`. Continue button stays disabled until both fields are filled. Error message updated to "No account found. Please check your username and email." on 404.
- **FamilyMemberScreen — ownerID fix**: `createMember` now passes `profile?.id` as `ownerID`; full profile fetched via `getUserByUsername` on mount and stored in state.
- **AccountSettingsScreen — live profile card**: fetches real profile via `getUserByUsername` on mount; displays `display_name` (falls back to `username`), role derived from `is_house_owner`, and `email`. Shows an `ActivityIndicator` while the profile loads.
- **LoginScreen — Sign Up validation**: added live username rule checklist (min 3 chars, no spaces, alphanumeric/underscore only) and password strength checklist (8+ chars, uppercase, lowercase, number, special char) shown as the user types in signup mode. Validation also runs in `handleSubmit` before the API call.
- **api.js — request() hardening**: removed `body['auth_token'] = token` block that crashed all authenticated GET requests (no body) with `TypeError: Cannot set properties of undefined`; error responses now parse the JSON `message` field so backend error text surfaces in the UI instead of raw status codes; password fields (`password`, `new_password`, `confirm_new_password`, `current_password`) are masked as `***` in `[API →]` logs; `getDeviceActivities` endpoint corrected to `GET /api/device-data`; `resetPassword` endpoint corrected to `POST /api/auth/forgot-password/reset` and now includes `email` in the body; `updateThresholds` PUT body now includes `auth_token` inline (`{ ...data, auth_token: token }`) as required by the backend — fixes 422 on save.
- **AuthContext — error propagation**: `signIn` re-throws after setting context error so `LoginScreen`'s catch block receives the actual backend message and displays it inline.
- **mockData.js**: all `last_record_time` / `last_recorded_at` timestamps now generated at module load time via `minsAgo(n)` so devices and sensors always appear online in DEV_MODE; `light-pwm` device fixed (`value: null` → `'60'`, null timestamp → `minsAgo(2)`).
...
- **HorizontalSlider fix (HomeScreen + DevicesScreen)**: replaced `locationX` in `onPanResponderMove` (unreliable during drag) with `gestureState.dx` applied as a delta from the value captured at touch-down (`startVal` ref). Added local `localVal` state so the fill and thumb update inside the slider without waiting for a parent re-render; `onChange` (which triggers `setDevices` and the API call) is now called only on **release**, eliminating the per-pixel re-render cascade that caused visible lag.
